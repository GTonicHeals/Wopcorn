 ...
 *
 * The discovery rails moved down here as "Browse": the feed is friends-only and
 * never contains your own activity, so an empty feed is the *normal* state for
 * someone who has not added anyone yet. That case needs somewhere to go — a
 * prominent link to `/friends` — and something to do meanwhile.
 *
...
<template>
  <div>
    ...
    <template v-else>
      ...
      <template v-else>
        <ul class="feed">
          <li v-for="item in items" :key="item.id" class="feed__row">
            <FeedItem :item="item" />
          </li>
        </ul>
    ...
    <!-- Always present: somewhere to go when the feed has nothing to say. -->
...
.feed__row {
  min-width: 0;
  /* A long feed only lays out what is near the viewport (NFR-2). */
...
@media (min-width: 900px) {
  /*
   * A timeline, not a grid: one 158px card per item leaves the full content
   * width empty, and the right-aligned timestamp ends up half a screen from the
   * name it belongs to. Capping the column keeps each item reading as one line.
   */
  .feed {
    max-width: 560px;
  }
